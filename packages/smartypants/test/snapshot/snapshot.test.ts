...
test("hidden files", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const patch = await Snapshot.patch(before!)
      expect(patch.files.map(normalize)).toContain(`${tmp.path}/.hidden`)
      expect(patch.files.map(normalize)).toContain(`${tmp.path}/.gitignore`)
      expect(patch.files.map(normalize)).toContain(`${tmp.path}/.config`)
    },
  })
})
...
test("snapshot state isolation between projects", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const __files1 = patch1.files.map(normalize)
      expect(__files1).toContain(`${tmp1.path}/project1.txt`)
    },
  })
